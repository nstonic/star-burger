...
def get_orders_with_available_restaurants(orders: QuerySet, restaurant_menu_items_with_places: QuerySet) -> QuerySet:
    ...
    for order in orders:
        available_restaurants = set.intersection(
            all_restaurants,
            *group_restaurants_by_product(order, restaurant_menu_items_with_places)
        )
...
def add_places_to_menu_items(restaurant_menu_items: QuerySet) -> QuerySet:
    ...
    places = get_places_for_addresses(restaurants_addresses)
...
def add_places_to_orders(orders: QuerySet) -> QuerySet:
    ...
    places = get_places_for_addresses(orders_addresses)
    for order in orders:
...
def get_places_for_addresses(addresses: Iterable[str]) -> list[Place]:
    places = []
    new_places = []
    ...
    places.append(Place.objects.bulk_create(new_places))
    return places
